...
function saveOptions() {
    ...
    var redirectMode = "";
    var radios = document.getElementsByName('redirect_mode');
    for (var i = 0, length = radios.length; i < length; i++) {
        if (radios[i].checked) {
          redirectMode = radios[i].value;
          break;
        }
      }

    const proxyUrl = $('#proxy_suffix').value;

    const sites = $$('#proxy_sites input').reduce(function (memo, inputEl) {
        if (inputEl.checked) {
...
function renderOptions() {
    extensionApi.storage.sync.get({
        ...
        for (const key in defaultSites) {
            ...
            $('#proxy_sites').appendChild(labelEl);
        }
        ...
        const proxyUrl = items.proxyUrl;
        $('#proxy_suffix').value = proxyUrl

        const redirectMode = items.redirectMode;

        var radios = document.getElementsByName('redirect_mode');
        for (var i = 0, length = radios.length; i < length; i++) {
            if (redirectMode == radios[i].value) {
